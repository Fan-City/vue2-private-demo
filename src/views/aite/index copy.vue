<template>
  <div id="app">
    <img
      class="logo"
      src="http://ww3.sinaimg.cn/large/006y8lVagw1fbhebeyucrj305k05kmx4.jpg"
    />
    <h1>{{ msg }}</h1>
    <h2>At Someone</h2>
    <ul class="at-some">
    ...
    <div class="area-box">
      <textarea
        v-model="textValue"
        @click="bindClick"
        @keyup.delete="bindDel"
        @keyup.left="bindLeft"
        @keyup.right="bindClick"
      >
      </textarea>
    </div>
    <pre>{{ peoplePos }}</pre>
...
export default {
  ...
  methods: {
    ...
    bindDel() {
      ...
      if (inPos) {
        textarea.value =
          tmpStr.substring(0, inPos.start + 1) +
          tmpStr.substring(inPos.end - 1, tmpStr.length);
    ...
    updatePos(val) {
      ...
      console.log(this.peoplePos, this.textValue);
    }
  },
};
</script>
